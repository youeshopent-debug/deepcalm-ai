...
function buildNotifyHtml(subscriber: Subscriber): string {
  return `<!DOCTYPE html><html lang="en"><head><meta charset="utf-8"><title>New DeepCalm Subscriber</title></head><body style="margin:0;padding:0;background-color:#0f1421;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif"><table width="100%" cellpadding="0" cellspacing="0"><tr><td align="center" style="padding:40px 20px"><table width="480" cellpadding="0" cellspacing="0" style="background:linear-gradient(135deg,#1a2238,#141a2e);border-radius:16px;border:1px solid rgba(126,184,255,0.15)"><tr><td style="padding:32px"><h2 style="color:#e8edf5;font-size:18px;margin:0 0 16px">📩 New Subscription</h2><table cellpadding="8" cellspacing="0" style="color:#8892b0;font-size:13px;width:100%"><tr><td style="color:#5a6484;width:100px">Email</td><td style="color:#e8edf5;word-break:break-all">${subscriber.email}</td></tr><tr><td style="color:#5a6484">Time</td><td style="color:#e8edf5">${subscriber.subscribedAt}</td></tr><tr><td style="color:#5a6484">Language</td><td style="color:#e8edf5">${subscriber.lang || "—"}</td></tr></table><hr style="border:none;border-top:1px solid rgba(126,184,255,0.1);margin:20px 0"><p style="color:#5a6484;font-size:11px;margin:0">DeepCalm AI · Midnight Sanctuary</p></td></tr></table></td></tr></table></body></html>`
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!transporter) {
      console.warn("[subscribe] SMTP not configured — email stored but not sent")
      return NextResponse.json({ ok: true, message: "subscribed_no_email" })
    }

    const mailFrom = process.env.MAIL_FROM || "DeepCalm AI <noreply@deepcalm-ai.com>"
    const mailTo = process.env.MAIL_TO || "alanlsl8208@gmail.com"
