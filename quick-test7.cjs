...
(async () => {
  ...
  page.on('pageerror', err => logs.push({ type: 'error', text: 'PAGE_ERROR: ' + err.message.substring(0, 300) }));
  page.on('requestfailed', req => logs.push({ type: 'error', text: 'REQ_FAIL: ' + req.url().substring(0, 120) + ' -> ' + (req.failure()?.errorText || '') }));
  await page.goto('https://deepcalm-ai.com/zh', { waitUntil: 'load', timeout: 60000 });
  ...
  console.log('BODY_TEXT_PREVIEW:', allText.substring(0, 500));
